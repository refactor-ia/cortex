...
// LoadCatalogFS loads a catalog from an fs.FS tree using the same admission rules as disk loading.
func LoadCatalogFS(assets fs.FS, catalogManifestPath string, policy AdmissionPolicy) (LoadedCatalog, error) {
	...
	data, err := readRegularFSFile(assets, catalogManifestPath)
	if err != nil {
	...
	loaded := LoadedCatalog{Manifest: manifest, Families: make([]LoadedFamily, 0, len(manifest.Families))}
	capabilityIDs := make(map[string]struct{})
	for _, reference := range manifest.Families {
		family, err := LoadFamilyFS(assets, reference.ManifestPath, policy)
		if err != nil {
